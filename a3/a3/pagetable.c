...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		pgtbl_entry_t * pte_to_evict = coremap[frame].pte;
		unsigned int evict_copy = pte_to_evict->frame;

		pte_to_evict->frame = pte_to_evict->frame & (~PG_VALID);

		if ((evict_copy & PG_DIRTY) == PG_DIRTY) {
			unsigned int ptevict_framenum = evict_copy >> PAGE_SHIFT; // only if the strict frame number is the input!
			int swapout = swap_pageout(ptevict_framenum, pte_to_evict->swap_off);
			pte_to_evict->frame = pte_to_evict->frame | PG_ONSWAP;
			pte_to_evict->swap_off = swapout;

			evict_dirty_count++; // a dirty eviction
		} else {
			evict_clean_count++; // a clean eviction
		}
	}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgtbl_entry_t *pgtb = (pgtbl_entry_t *) (pgdir[idx].pde & PAGE_MASK);

	unsigned pgtbIdx = PGTBL_INDEX(vaddr);
	p = pgtb + pgtbIdx;

	// Check if p is valid or not, on swap or not, and handle appropriately

	unsigned int frameAddress = p->frame;
	int isValid = (frameAddress & PG_VALID) == PG_VALID;
	int onSwap = (frameAddress & PG_ONSWAP) == PG_ONSWAP;
	int frameAlloc;
	...
	if (!isValid) {
		...
		miss_count++; // INVALID -> NOT IN MEMORY -> MISS

		if (!onSwap) {

			frameAlloc = allocate_frame(p);
			init_frame(frameAlloc, vaddr);
			p->frame = frameAlloc << PAGE_SHIFT;
			p->frame = p->frame | PG_DIRTY; // SETS THE DIRTY BIT BECAUSE ALLOCATED FOR FIRST TIME


		} else if (onSwap) {
			frameAlloc = allocate_frame(p);
			if (swap_pagein(frameAlloc, p->swap_off) != 0) {
				fprintf(stderr,"swap_pagein failure");
				exit(0);
			}
			p->frame = frameAlloc << PAGE_SHIFT;
