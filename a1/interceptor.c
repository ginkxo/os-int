...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//have to check that the arguments are valid
	if (syscall < 0 || syscall >= NR_syscalls || syscall == MY_CUSTOM_SYSCALL) {
		return -EINVAL;
	} else { //arguments are valid

		if(current_uid() == 0) { //we are root
			...
			if (pid < 0 || (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL && \
				...
				if (cmd == REQUEST_START_MONITORING) {
					if (table[syscall].monitored == 2) { //already being monitored
						return -EBUSY;

						// if table[syscall].monitored == 1 -> need to check if our pid is in the list
						...
						spin_lock(&my_table_lock); //added the cases here for pid == 0 or not
						if (pid == 0) {
				...
				if (cmd == REQUEST_STOP_MONITORING) {
					if(table[syscall].monitored == 0 || \
					   table[syscall].intercepted == 0) { //not monitored OR intercepted
						return -EINVAL;
					} else {
			...
			if (cmd == REQUEST_SYSCALL_INTERCEPT || \
			    cmd == REQUEST_SYSCALL_RELEASE || pid == 0) {
				return -EPERM;
			}
...
static int init_function(void) {
	...
	//lock the sys_call_table because we are entering the critical section
	spin_lock(&sys_call_table_lock);

	//allow writing to the table
	set_addr_rw((unsigned long) sys_call_table);

	//save MY_CUSTOM_SYSCALL in orig_custom_syscall
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];

	//make MY_CUSTOM_SYSCALL point to my_syscall
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;

	//save __NR_exit_group to orig_exit_group
	orig_exit_group = sys_call_table[__NR_exit_group];

	//make __NR_exit_group point to my_exit_group
	sys_call_table[__NR_exit_group] = my_exit_group;

	//make table read-only again
	set_addr_ro((unsigned long) sys_call_table);

	//unlock the sys_call_table because we are exiting the critical section
	spin_unlock(&sys_call_table_lock);

	//initialize all the variables in the data structure
...
static void exit_function(void){
	...
	//unlock the sys_call_table
	spin_unlock(&sys_call_table_lock);

	//destroy monitored pid list
	for(i = 0; i < NR_syscalls; i++) {
		spin_lock(&my_table_lock);
		destroy_list(i);
		spin_unlock(&my_table_lock);
