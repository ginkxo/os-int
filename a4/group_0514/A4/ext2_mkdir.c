...
int allocateDirBlocks(int dirInodeNum, int dirParentInodeNum) {
	...
	selfLink->inode = dirInodeNum + 1;
	selfLink->name_len = 1;
	selfLink->rec_len = 12; // FIX THIS LATER
	selfLink->file_type = EXT2_FT_DIR;
	strncpy(selfLink->name, selfLnkName, EXT2_NAME_LEN);

	struct ext2_dir_entry * parentLink = (struct ext2_dir_entry *)(disk + EXT2_BLOCK_SIZE * dirInode->i_block[0] + selfLink->rec_len);

	parentLink->inode = dirParentInodeNum + 1;
	parentLink->name_len = 2;
	parentLink->rec_len = EXT2_BLOCK_SIZE - 12; // FIX THIS LATER
	parentLink->file_type = EXT2_FT_DIR;
...
int goodPath(char * basePath) {
	...
	while (parentDir != NULL && inodeID != -1) {
		...
		//// LOOK HERE TO SEE HOW TO LOOP THROUGH DIR ENTRIES:
		...
		//// LOOK UP THERE TO SEE HOW TO LOOP THROUGH DIR ENTRIES
...
void makedir(char * path) {
	...
	duoflag dflag = checkParentDirEntListSpaceExists(inodeIndexOfParentDir, directoryToCreateName);

	//struct ext2_inode * parentDirInode2 = (struct ext2_inode *)(inodeTable + inodeIndexOfParentDir * sizeof(struct ext2_inode));
	// for test purposes

	//printf("blockloc: %d \n", parentDirInode2->i_block[0]);
	...
	//struct ext2_inode * parentDirInode3 = (struct ext2_inode *)(inodeTable + inodeIndexOfParentDir * sizeof(struct ext2_inode));
	// for test purposes

	//printf("blockloc: %d \n", parentDirInode3->i_block[0]);

	/////// THIS FUNCTION IS THE PROBLEM VVV

	if (allocateDirBlocks(newDirInodeIndex, inodeIndexOfParentDir) < 0) {
	...
	// for test purposes
	//struct ext2_inode * parentDirInode4 = (struct ext2_inode *)(inodeTable + inodeIndexOfParentDir * sizeof(struct ext2_inode));
	// for test purposes

	//printf("blockloc: %d \n", parentDirInode4->i_block[0]);
